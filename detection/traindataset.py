...
from keras.layers import Input, Conv2D, MaxPooling2D, LSTM, Flatten, Dense, Dropout, TimeDistributed, ConvLSTM2D,BatchNormalization,MaxPooling3D
...
def build_hybrid_model(input_shape):
    ...
    # x = ConvLSTM2D(32, kernel_size=(3, 3), activation='relu', padding='same', return_sequences=True)(inputs)
    # x = BatchNormalization()(x)
    # x = Dropout(0.3)(x)
    # # x = MaxPooling3D(pool_size=(1, 2, 2))(x)  # First pooling layer

    # # Second ConvLSTM2D layer
    # x = ConvLSTM2D(64, kernel_size=(3, 3), activation='relu', padding='same', return_sequences=True)(x)
    # x = BatchNormalization()(x)
    # x = Dropout(0.3)(x)
    # # x = MaxPooling3D(pool_size=(1, 2, 2))(x)  # Second pooling layer

    # # Third ConvLSTM2D layer
    # x = ConvLSTM2D(128, kernel_size=(3, 3), activation='relu', padding='same', return_sequences=False)(x)
    # x = BatchNormalization()(x)
    # x = Dropout(0.3)(x)

    #  # Reshape to 4D for MaxPooling2D
    # # x = tf.reshape(x, (-1, x.shape[1], x.shape[2], x.shape[3]))  # Reshape to (batch_size, height, width, channels)
    # # x = MaxPooling2D(pool_size=(2, 2))(x) # Third pooling layer

    # # Flatten and prepare for fully connected layers
    # x = Flatten()(x)
